...
Execs create more managers with the Manager Charter (§7) when a function has recurring volume — and absorb/retire them when it doesn't (§8).
...
| Chartering a new manager | owning exec | Atlas | No |
| Retiring/absorbing a manager | owning exec | Atlas | No |
...
| `.cursor/company/journal/<dept>.md` | Each exec, own file only | Managers likewise: `journal/mgr-<function>.md` |
...
A manager exists when its charter and skill exist. To create one: write the charter, create `.cursor/skills/mgr-<function>/SKILL.md` (mirror the seeded managers' structure), seed `playbooks/<function>.md` (v0, UNVALIDATED) and `journal/mgr-<function>.md`, add it to the §1 roster, announce in `#boardroom`. All by PR.
