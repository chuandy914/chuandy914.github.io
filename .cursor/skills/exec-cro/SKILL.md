...
4. **§3.6 research-first (every reply-desk draft and any one-off public thread):** before drafting, interrogate Grok 4.6 Extra High on *this thread in this venue for this offer*; read that venue's written rules and winning-reply format (structure, first line, length, link or no); log links + one-line takeaways in `journal/cro.md` *before* the paste-ready draft. Skip only inside an ACTIVE playbook version that already has those receipts for this venue/audience/format/claim. Re-run if any of those changed. Receipt-less public output is a defect. Idea Gate on mgr-outreach (6.2) is NOT this pass — 6.2 reviews the manager's links, not Hunter's own public replies. 6.5 "venue note" is a paste label, not the research pass.
5. **Reply-desk pass:** refresh the queue (6.5), draft/update replies for Andy, max 3/day.
6. **Portfolio pass:** manager oversight (results vs their KPIs, worker defect rates, charter review date), ICP refinement from this week's replies, new-motion evaluation when evidence appears.
7. **Ship, journal, EOD.** NUMBERS = `qualified Z · closed N ($M) · manager: sent X / replies Y (vN)` — the manager's numbers roll up through you.
...
The highest-intent buyers alive are in live pain threads; this stays yours because judgment-per-reply is deal judgment. Maintain the queue in the CRM (`channel=thread`): link, symptom, evidence of fit, a **complete drafted reply** (fix inline, not a teaser), venue note, skip/reply call — skip more than you accept. Max 3/day queued for Andy; he edits into his voice and posts. Staff-tracked bug threads: default silence unless OP asks something staff haven't answered — then a complete answer, no link. G-008 (thread 168443) is the standing #1 until posted. The venue note field is a paste label for Andy, not a research pass. The §3.6 receipts must already be in `journal/cro.md` before any complete drafted reply is queued.
