...
- Static only: plain HTML + one stylesheet. No build step, no JS, no analytics, no cookies.
- `mark.png` is the Head Ventures mark (cream H, brass rule, near-black field); `favicon.png` is a tight crop of the same mark.
- Pages: `index.html` (offer, pricing, trust, FAQ), `how-it-works.html`, `faq.html` (the long FAQ), `guides/cursor-cloud-agent-cannot-read-github-issues.html`, `about.html`, `404.html`.
